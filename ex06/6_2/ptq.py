...
def main():
    ...
    num_samples = 1000 # generally a small subset is enough for calibration set
    calibration_indices = torch.randperm(len(cifar10_dataset_train))[:num_samples]
    calibration_dataset = Subset(cifar10_dataset_train, calibration_indices)
    calibration_loader = DataLoader(calibration_dataset, batch_size=64, shuffle=False)


    #train floating point model 
    VGG11_ = VGG11()
    pretrain_models = [VGG11_]
    ...
    model_path = f"VGG11_trained_{args.epochs}_epochs.pth"
    ...
    for bit_width in bit_widths:
        ...
        # initialize
        QVGG11 = QuantVGG11(bit_width, bit_width)
        # transfer parameters
        QVGG11.load_state_dict(state_dict)
        QVGG11 = QVGG11.to(device)

        # calibration missing

        testloss, testacc = test(QVGG11, device, test_loader, 1)

        print(f'Test set: Average loss: {testloss:.4f}, Accuracy: {testacc} %)')

        accurracies.append(testacc)
    
    plt.figure()
    plt.plot(bit_widths, accurracies)
    plt.xlabel("Bitwidth")
    plt.ylabel("Test accuracy (%)")
    plt.title("Quantization (no calibration)")
    plt.legend()
    plt.savefig("Quantization_no_calibration" + ".png")
