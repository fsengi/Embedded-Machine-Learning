...
class VGG11_GN(nn.Module):
    '''VGG11 module with Group Normalization'''
    def __init__(self, num_groups=1):
        super().__init__()
        self.layers = self._make_layers(num_groups)
        self.name = f"VGG11_GN({num_groups})"
    
    def _make_layers(self, num_groups):
        layers = [

            nn.Conv2d(in_channels=3, out_channels=64, kernel_size=3, padding=1, bias=False),
            nn.GroupNorm(num_channels=64, num_groups=num_groups),
            nn.ReLU(),
            ...
            nn.GroupNorm(num_channels=512, num_groups=num_groups), 
            nn.ReLU(),
            ...
            # don't use Normalization for classification part of network
...
# possible TODO: block of [Conv, Norm, ReLU]
class VGG11_BN(nn.Module):
    '''VGG11 module with Batch Normalization'''
    def __init__(self):
        super().__init__()
    ...
    def _make_layers(self):
        layers = [
            ...
            nn.Conv2d(in_channels=3, out_channels=64, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(64), # as many params as prev layer output channels
            nn.ReLU(),
            ...
            nn.Flatten(end_dim=-1),
            # don't use BN for classification part of network
            nn.Linear(in_features=512, out_features=4096),
            nn.ReLU(),
            nn.Linear(in_features=4096, out_features=4096),
            nn.ReLU(),
...
class VGG11(nn.Module):
    '''VGG11 module without Normalization'''
    def __init__(self):
        super().__init__()
        self.layers = self._make_layers()
        self.name = "VGG11"
    ...
    def _make_layers(self):
        layers = [
            ...
            nn.GroupNorm(num_channels=128, num_groups=64), 
            nn.ReLU(),

            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(128, 256, kernel_size=3, padding=1),
            nn.GroupNorm(num_channels=256, num_groups=128), 
            nn.ReLU(),
            ...
            nn.GroupNorm(num_channels=512, num_groups=256), 
            nn.ReLU(),
...
def main():
    ...
    VGG11_ = VGG11()
    VGG11_BN_ = VGG11_BN()
    VGG11_GN_1 = VGG11_GN(1)
    VGG11_GN_16 = VGG11_GN(16)
    VGG11_GN_32 = VGG11_GN(32)

    models = [VGG11_, VGG11_BN_, VGG11_GN_1, VGG11_GN_16, VGG11_GN_32]
    ...
    for model in models:
        ...
        plot_one_model(dataDict,epochs=30, title=model_name)
